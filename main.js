...
function login(arr, userName, pass) {
    for (let i = 0; i < arr.length; i++) {
        if (userName != arr[i].Username && pass != arr[i].password) {
            console.log('user not found!');
        } else if ((userName != arr[i].Username && pass == arr[i].password) || (userName == arr[i].Username && pass != arr[i].password)) {
            console.log('incorrect username or password');
        } else if ((pass == arr[i].password && userName == arr[i].Username) && arr[i].isLogged == false ){
            console.log('user did not logged in to log out!');
        } else if ((pass == arr[i].password && userName == arr[i].Username) && arr[i].isLogged == true  ) {
...
function LogOut(arr, userName) {
    for (let i = 0; i < arr.length; i++) {
        if (arr[i].Username !== userName) {
            console.log('user not found');
        } else if ((arr[i].Username === userName) && arr[i].isLogged == false) {
            console.log( 'user did not logged in to log out!');
        } else if ((arr[i].Username === userName) && arr[i].isLogged ==  'successfully') {
            console.log( 'user did not logged in to log out!');
        }
    }
}